...
typedef void (*c_method)(void*,struct entity*);
...
#define declare_draw(name, body)\
    void name ## _draw(void *accessor, entity *parent) {\
        name *self = accessor;\
        body\
    }
#define declare_tick(name, body)\
    void name ## _tick(void *accessor, entity *parent) {\
        name *self = accessor;\
        body\
    }
